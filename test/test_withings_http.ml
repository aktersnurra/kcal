...
let integration store =
  ...
  Http_adapter.{ oauth = Withings_oauth.make ~store ~now:(fun () -> Option.get (Ptime.of_float_s 1_800_000_000.)); client = (module Client); token_key = Bytes.make 32 'k'; sync = (fun _ _ -> Ok ()); callback_url = "https://kcal.example.com/withings/webhook" }
...
let test_webhook_boundaries () =
  ...
  Alcotest.(check int) "malformed" 400 (request store ~headers:[ ("content-type", "application/json") ] `POST "/withings/webhook" "{}").status;
  Alcotest.(check int) "unknown identity" 404 (request store ~headers:[ ("content-type", "application/json") ] `POST "/withings/webhook" "{\"userid\":\"unknown\"}").status
